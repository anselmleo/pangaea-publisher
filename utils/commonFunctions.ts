function formatter(stringData: string, dataObject: object) {
...
function transformMobile(mobile: string) {
  let newMobile = '';
...
function getStartOfDay() {
  ...
  let options: object = { day: 'numeric' };
  let day: any = nd.toLocaleDateString(undefined, options);
  if (day < 10) day = '0' + day;
  options = { month: 'numeric' };
  let month: any = nd.toLocaleDateString(undefined, options);
...
function getDateFromEpoch(startTime: any) {
  let sign = '+';
  let offsetHour = 1;
  let offsetMin = 0;
